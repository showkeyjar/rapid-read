...
# 提前定义check_ollama_health，避免NameError
...
def generate_final_summary(section_summaries: list[str]) -> str:
    ...
    for attempt in range(3):
        try:
            prompt = (
                "你是一名中文专业文献总结助手，所有输出必须为简体中文。\n"
                "请根据以下章节摘要，用简体中文生成一个结构化的最终总结（500字以内）：\n\n"
                f"{combined}"
            )
            response = requests.post(
                f"{os.getenv('OLLAMA_API_BASE')}/api/generate",
            ...
            if response.status_code == 200:
                ...
                # 检查是否为英文
                if re.search(r'[a-zA-Z]{8,}', result) and not re.search(r'[\u4e00-\u9fa5]', result):
                    raise ValueError("模型未输出中文摘要")
                return result.strip()
            else:
                logger.error("最终摘要生成失败，状态码: %s", response.status_code)
        except Exception as e:
...
def _short_text(text, max_lines=5):
...
def display_drilldown_summary(result):
    ...
    with col1:
        if st.button("← 上一层级", key="btn_up_level_drill") and st.session_state.current_level > 1:
            st.session_state.current_level -= 1
            st.rerun()
    with col2:
        if st.button("重置到第一层", key="btn_reset_level_drill"):
            st.session_state.current_level = 1
            st.rerun()
    ...
    if st.session_state.current_level == 1:
        st.markdown("## 最终摘要")
        st.markdown(_short_text(result['final_summary']))
        st.markdown("## 章节摘要")
        for idx, section in enumerate(result['sections']):
            short_content = _short_text(section['content'])
            st.markdown(f"**{section['title']}**\n\n{short_content}")
            if st.button(f"下钻到{section['title']}", key=f"drill_section_{idx}_lv1"):
    ...
    # 2级：章节下分块摘要
        ...
        st.markdown(_short_text(section['content']))
        st.markdown("---")
        st.markdown("#### 分块摘要")
        for i, idx in enumerate(section['chunk_ids']):
            chunk_summary = next((s for j, s in result['summaries'] if j == idx), None)
            if chunk_summary:
                short_chunk = _short_text(chunk_summary)
                st.markdown(f"**分块 {idx+1}**\n{short_chunk}")
                if st.button(f"下钻到分块 {idx+1}", key=f"drill_chunk_{idx}_lv2"):
...
col1, col2 = st.columns(2)
with col1:
    if st.button("← 上一层级", key="btn_up_level_main") and st.session_state.current_level > 1:
        st.session_state.current_level -= 1
        st.rerun()
with col2:
    if st.button("重置到第一层", key="btn_reset_level_main"):
        st.session_state.current_level = 1
        st.rerun()
